...
#data=实际的字典,ws为data转化的WordSequence
#作用将句子data随机取batch_size句转化为[句子向量,向量长度]的矩阵
...
#作用将句子data分割成n_bucket份，每份随机取batch_size句转化为[句子向量,向量长度]的矩阵
def batch_flow_bucket(data, ws, batch_size, raw=False, add_end=True, n_bucket=5, bucket_ind=1, debug=False):
    ...
    :param bucket_ind: 哪一个维度的输入作为bucket的依据，解压后为（[句子][向量]）,所以bucket_ind必须为1
    :param debug:
    :return:
    """
    #解压后的结果为[句子][向量]一一对应
