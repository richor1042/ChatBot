...
class SequenceToSequence(object):
    ...
    def __init__(self,
        ...
        beam_width:
            beam_width是beamsearch的超参数，用于解码
        ...
        bidirectional: encoder中的rnn是否为双向
        time_major:
        ...
        pretrained_embedding: 是否使用预训练的embedding
        """
        ...
        assert self.cell_type in ('gru', 'lstm'), 'cell_type 应该是 GRU 或者 LSTM'

        if share_embedding:
            assert input_vocab_size == target_vocab_size, 'if share_embedding is true,两个vocab_size必须一样'

        assert mode in ('train', 'decode'), 'mode 必须是 "train" 或 "decode" 而不是 "{}"'.format(mode)

        assert dropout >= 0.0 and dropout < 1.0, '0 <= dropout < 1'

        assert attention_type.lower() in ('bahdanau', 'luong'), \
            '''attention_type 必须是 "bahdanau" 或 "luong" 而不是 "{}"'''.format(attention_type)

        assert beam_width < target_vocab_size, \
            'beam_width {} 应该小于 target vocab size {}'.format(beam_width, target_vocab_size)
    ...
    def build_decoder_cell(self, encoder_outputs, encoder_state):
        ...
        if self.use_beamsearch_decode:
            encoder_outputs = seq2seq.tile_batch(encoder_outputs, multiplier=self.beam_width)
